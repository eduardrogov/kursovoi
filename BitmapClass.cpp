// BitmapClass.cpp: реализация класса BitmapClass class.
//
//////////////////////////////////////////////////////////////////////
...
bool BitmapClass::Open( CString cFilePath_i)
{
    ...
    fseek( fp, 14, 0 ); // размер BITMAPFILEHEADER
    ...
    fseek( fp, 0, 0 ); // поиск происхождения ;
    ReadPixelData( fp );
    //Create32BitData();
    return true;
}
...
void BitmapClass::Draw( HWND hwnd, int nXStart_i, int nYStart_i, int nWidth_i, int nHeight_i )
{
    ...
    RECT Rect; // попытка отрегулировать размер окна
    ...
    if( 0 == m_BMPInfoHeader.biHeight )
    {
        ...
        m_BMPInfoHeader.biSize = sizeof(m_BMPInfoHeader); // размер этой структуры
    ...
    BITMAPINFO biBitMapInfo; // Мы поставили это, чтобы захватить то, что мы хотим
    biBitMapInfo.bmiHeader = m_BMPInfoHeader;

    HDC iDC = CreateCompatibleDC(0); // создание места для сохранения изображения 

    HBITMAP iBitmap = CreateDIBSection( iDC, &biBitMapInfo, DIB_RGB_COLORS, 0, 0, 0); // создание dib раздела для dc
    SelectObject(iDC, iBitmap); // назначение dib раздела для dc
    ...
    SetDIBitsToDevice( iDC, 0, 0, biBitMapInfo.bmiHeader.biWidth, biBitMapInfo.bmiHeader.biHeight,
                       ...
                       DIB_RGB_COLORS); // установка новых dibs для dc
  ...
  //  if((( nWidth_i - 3)  < biBitMapInfo.bmiHeader.biWidth ) ||
   ...
   // else
    ...
    DeleteObject( iBitmap ); // удаление объекта
    ::ReleaseDC( hwnd, hdc );
}
